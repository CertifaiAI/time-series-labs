 ...
 *
 * In this example, we will show you the method in performing time series prediction using time series data stored in a CSV file.
 * We can perform the prediction in following steps:
 *  1. Initialize the data
 *      Here, we will be using CSVSequenceRecordReader in loading the data instead of using a self written class as shown
 *      in univariate time series examples.
 *      Here, we don't need to preprocess the data manually via sliding window since all data have been preprocessed.
 *
 *      All data are stored in CSV formats, which can be found in "DataImportMultivariate" under "resources" folder.
...
public class CSVDataLoader {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        */
        //Training set
        File baseDir = new ClassPathResource("datasets").getFile();
        File baseDir2 = new File( baseDir, "DataImportMultivariate");
        File featureDir = new File(baseDir2, "feature");
        File labelDir = new File(baseDir2, "label");
